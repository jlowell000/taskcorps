---
name: bootstrap
description: Used by the pm agent on /scrum-init to wire a target project into the scrum team: copy the team, run project discovery, seed .team/ state, register with the baseline, and establish a green test baseline.
---
...
6. **Register** — write `.team/federation/baseline.md` into the target: its **primary baseline**
   (path or git URL of the baseline repo) and the installed version. Then register the target in
   the baseline's `.team/federation/registry.md` (id, path, installed version) and take a
   `catalog/<project>/<version>/` snapshot of what was installed so future releases diff
   correctly. Registration is two-sided; both sides are required for the federation loop to work.
7. **Report** — summarize to the human: what was installed, discovered facts, any open
...
- Same-team portability: after bootstrap, `/scrum`, `/status`, `/retro`, and the federation
  commands must all work because the whole install is self-consistent.