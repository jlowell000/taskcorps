...
1. `.team/context/pr-capabilities.md` — confirm PR creation is enabled; read the remote URL, default branch, and provider.
...
2. **Detect provider.** Parse the `Provider:` field from `pr-capabilities.md`. Supported providers: `github` (gh CLI), `gitlab` (glab CLI), `gitea` (gitea CLI or manual).

3. **Create a release branch.** Name it `retro-YYYYMMDD-<slug>` where `<slug>` is a short kebab-case summary of the release (e.g., `retro-20260821-remove-federation`). Check it out from the current default branch.

4. **Apply approved proposals.** For each proposal the human approved:
...
5. **Commit and push.** Stage only the files changed by the proposals. Commit with a message listing the proposal filenames. Push the branch to origin.

6. **Open a PR.** Use the detected provider's CLI:
   - `--base` / `--target` set to the configured default branch from `pr-capabilities.md`
   - `--head` / `--source` set to the release branch name
   ...
   - Labels: `retro` if that label exists on the repo (non-fatal if it doesn't)

7. **Record the PR URL.** Append it to `.team/checkpoints/<run-id>.md` or write a new checkpoint so the human can find it.
...
Return the PR URL and a one-line summary of what changed. If PR creation failed, report the exact CLI error so the human can act.
