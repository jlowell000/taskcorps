...
2. CREATE the run id: `YYYYMMDD-<objective-slug>`; write it to `.team/checkpoints/README.md` as the `latest` pointer.
3. DECOMPOSE the objective into backlog items (use the `decompose` skill) with **run-scoped ids** (`<run>-T<n>`). Create `.team/tasks/<run>-T<n>/` folders with `brief.md`.
4. DISPATCH each task in dependency order through the team, gating every stage:
...
5. CHECKPOINT + ARCHIVE at every stage transition (use the `context-management` skill).
6. REPORT back to me plainly: what was delivered, what was deferred (queued for next run), what needs my decision.
