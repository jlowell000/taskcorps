...
#   1  conflicts remain / user declined / abort
...
if [ -f AGENTS.md ]; then
  if [ -n "$DRY" ]; then
    ...
    base_tmp=".team/federation/conflicts/AGENTS.md.upstream"
    ...
    if [ -z "$prev_cat" ]; then
      ...
      prev_tmp=".team/federation/conflicts/AGENTS.md.prev"
    ...
    if ask "surgically merge AGENTS.md (preserve local tail)"; then
      tmp=".team/federation/conflicts/AGENTS.md.ours"
      if scripts/merge-agents.sh "$base_tmp" "$prev_cat" AGENTS.md "$tmp"; then
        ...
        warn "AGENTS.md could not be auto-merged; see '$tmp' and .team/federation/conflicts/"
        exit 1
      fi
    else
...
# exit 1 so the caller knows the sync did not complete cleanly.
